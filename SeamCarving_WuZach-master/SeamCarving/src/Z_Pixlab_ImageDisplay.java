 ...
 */
public class Z_Pixlab_ImageDisplay extends JPanel implements Scrollable
{
   ...
   */
  public Z_Pixlab_ImageDisplay(Image theImage)
  {
    image = theImage;
    prefSize = new Dimension(image.getWidth(this),image.getHeight(this));
    widthCur = image.getWidth(this);
    heightCur = image.getHeight(this);
    sc = new SeamCarver(new SmC_Picture((BufferedImage) theImage));
   ...
   */
  public Z_Pixlab_ImageDisplay(Image theImage, int x, int y)
  {
    this(theImage);
    currentX = x;
    currentY = y;
  ...
  {
      System.out.println("onResize called: " + this.getWidth() + " x " + this.getHeight());
      
      int removeRows = heightCur - getHeight();
      int removeColumns = widthCur - getWidth();
      if (removeRows < 0)
          removeRows = 0;
      if (removeColumns < 0)
          removeColumns = 0;
      ...
      SmC_Picture outputImg = sc.picture();
      setImage(outputImg.getImage());
